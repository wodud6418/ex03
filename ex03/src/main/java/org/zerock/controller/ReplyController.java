...
public class ReplyController {
	...
	//consumes 통해보내온데이터 타입확인 틀리면 415(HttpStatus.UNSUPPORTED_MEDIA_TYPE)
	...
	public ResponseEntity<String> urlNew(@RequestBody  ReplyVO vo) {
			...
			if(count==1) {
			//댓글이 등록되면 success 문자를 보내고 상태는 200
	...
	public ResponseEntity<String> urlDelete(@PathVariable("rno") Long rno) {
		...
		if(count==1) {
			//삭제성공 success 문자를 보내고 상태는 200
					return new ResponseEntity<>("sucess",HttpStatus.OK);
			} else {	
			//삭제실패하면 아무것도 안보내고 418에러
